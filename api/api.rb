...
class ApiController < Sinatra::Application
  ...
  def crud_response(resource)
    if resource.nil?
      ...
      halt 400, { 'Content-Type' => 'application/json' }, { message: resource.errors }.to_json
    else
  ...
  namespace '/v1' do
    ...
    put '/records/:id' do |id|
      crud_response(
        UpdateResource.new(Record.find_by(id: id, user: @current_user), JSON.parse(request.body.read)['record'])
      )
    end
    ...
    delete '/cards/:id' do |id|
      crud_response Card.find_by(id: id, user_id: @current_user.id)&.desrtoy
    end
